...
class BReplayBuffer(Dataset):
    ...
    def __init__(self, obs_shape, action_shape, capacity, buffer_dir, batch_size, nstep,
        ...
        self.rewards = np.ones((capacity, 1), dtype=np.float32) * -1
        ...
        self.ep_idxes = np.ones(capacity, dtype=np.int) * -1
    ...
    def add_obs(self, obs):
        ...
        self.ep_idxes[self.idx] = self.ep_num
        self.obses[self.idx] = obs[:3].copy()
        self.num += 1
        self.ep_idxes[self.idx] = self.ep_num
        self.obses[self.idx] = obs[3:6].copy()
        self.num += 1
    ...
    def add(self, obs, action, reward, next_obs, done, done_bool, ep_len, env_label=0):
        ...
        if done:
            ...
            # self.idx changes, judge whether self.anchor_idx == self.idx
            # if self.full and self.anchor_idx == self.idx:
            #    if self.ep_end_idxs[self.valid_start_ep_num] == self.ep_start_idxs[self.valid_start_ep_num]:
            #        del self.ep_num_list[0]
            #        self.valid_start_ep_num = self.ep_num_list[0]
    ...
    def sample_batch_without_reward(self, batch_size=None):
        ...
        # sample indexes
        # all_the_same_ep = True
        indexes = list(range(min(self.capacity, self.num)))
        for i in range(len(indexes)):
            if self.ep_idxes[i] != self.ep_idxes[(i + 1) % len(indexes)]:
                # all_the_same_ep = False
                # for j in range(-self.nstep_rsd+1, 3):
                for j in range(0, 3):
                    indexes[(i + j) % len(indexes)] = -1
        # if all_the_same_ep:
            # indexes[0] = -1
            # indexes[1] = -1
            # for j in range(1, self.nstep_rsd + 1):
            #     indexes[-j] = -1

        indexes = np.array(indexes)
        idxs = np.random.choice(indexes[indexes != -1], batch_size)
        obs_idxs = idxs.repeat(3).reshape(-1, 3)  # (batch_size, 3)
        obs_idxs[:, 0], obs_idxs[:, 1] = obs_idxs[:, 0] - 2, obs_idxs[:, 1] - 1
        obs_idxs = obs_idxs.flatten()
    ...
    def sample_batch(self, batch_size=None):
        ...
        # transition['rew2'] = torch.as_tensor(self.rewards[idxs + 1], device=self.device).float()
        # transition['act2'] = torch.as_tensor(self.actions[idxs + 1], device=self.device).float()
        # assert torch.ge(transition['rew'], 0).all()
        # assert torch.ge(transition['rew2'], 0).all()
        return transition
    ...
    def sample_batch_with_rs(self, batch_size=None):
        ...
        for i in range(len(indexes)):
            if self.ep_idxes[i] != self.ep_idxes[(i + 1) % len(indexes)]:
                # all_the_same_ep = False
                for j in range(-self.nstep_rsd + 2, 3):
                    indexes[(i + j + len(indexes)) % len(indexes)] = -1
        # if all_the_same_ep:
            # indexes[0] = -1
            # indexes[1] = -1
            # for j in range(1, self.nstep_rsd + 1):
        ...
        transition = {k: torch.as_tensor(v, device=self.device).float() for k, v in batch.items()}
