...
_ACTION_REPEAT = {
    ...
    'dmc.hopper.stand': 4, 'dmc.cartpole.swingup_sparse': 8, 'dmc.fish.swim': 4,
    'dmc.point_mass.hard': 4, 'dmc.pendulum.swingup': 8,
    'carla.highway': 4
}
...
def compute_cl_loss(z1, z2, labels=None, mask=None, temperature=1.0, output_acc=False):
    similarity = cosine_similarity(z1, z2) / temperature # (output_dim, batch_size, batch_size)
    ...
    with torch.no_grad():
        if labels is None:
            ...
            target = F.one_hot(labels, similarity.size(1)).to(z1.device) # (batch_size, batch_size)
        pred_prob = torch.softmax(similarity, dim=-1) # (output_dim, batch_size, batch_size)
    ...
    loss = (similarity * diff).sum(-1).mean(-1) # (output_dim)
    if output_acc:
