...
def set_carla(image_size, action_repeat, frame_stack, render):
    # TODO: implement env.seed(args.seed) ?
    env = CarlaEnv(
            render_display=render,  # for local debugging only
            display_text=render,  # for local debugging only
            changing_weather_speed=0.1,  # [0, +inf)
            rl_image_size=image_size,
    ...
    obs_shape = (3 * frame_stack, image_size, image_size*5)
    pre_aug_obs_shape = (3, image_size, image_size*5)
    return train_envs, test_env, (obs_shape, pre_aug_obs_shape)
    
def set_dcs_multisources(image_dir, domain_name, task_name, image_size, action_repeat,
    ...
    for i in range(num_sources):
        ...
        env = make(None, domain_name, task_name, seed, image_size,
    ...
    test_env = make(image_dir, domain_name, task_name, seed, image_size,
                    action_repeat, frame_stack, background_dataset_path,
                    background_kwargs=background_kwargs,
                    camera_kwargs=camera_kwargs, color_kwargs=color_kwargs)
