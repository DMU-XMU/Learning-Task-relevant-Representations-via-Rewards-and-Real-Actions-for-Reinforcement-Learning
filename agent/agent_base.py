...
class AGENTBase(object, metaclass=abc.ABCMeta):
    ...
    def _init_extractor(self, env_name, obs_shape, extr_config):
        module_dict = init_extractor(env_name, obs_shape, self.device, extr_config)
    ...
    def augment(self, data, num_aug=2, name=''):
        ...
        data['aug_o'] = aug_o
        data['aug_o2'] = aug_o2
        if self.aux_task_name == 'rra':
            negative_sample_list = []
            for i in range(self.aux_task.negative_sample_num):
                negative_sample = torch.as_tensor(self.select_action(data['obs'], deterministic=False, to_numpy=False), device=self.device).float()
                negative_sample_list.append(negative_sample)
            data['negative_samples'] = torch.stack(negative_sample_list, dim=1) # (batch_size, negative_sample_num, act_dim)
    ...
    def select_action(self, obs, deterministic=False, tanh=True, to_numpy=True):
        ...
        if obs.ndim == 3:
            obs = obs.unsqueeze(0)
        s = self.extr(_handle_data(obs.view(-1, *obs.size()[-3:])))
    ...
    def update(self, replay_buffer, logger, step, save_log, batch_size=None):
        ...
        if self.aux_task_name == 'cresp':
            data1 = replay_buffer.sample_batch_with_rs(batch_size)
            data2 = replay_buffer.sample_batch_with_rs(batch_size)
        else:
            data1 = replay_buffer.sample_batch(batch_size)
            data2 = replay_buffer.sample_batch(batch_size)
    ...
    def auxiliary_update(self, data, logger, step, save_log):
        if self.aux_task is not None:
            ...
            aug_s2 = self.rl.actor.forward_trunk(self.extr(data['aug_o2']))
            loss, opt_dict, info_dict = self.aux_task.update_extr(data, aug_s, aug_s2, self.num_aug)
    ...
    def print_log(self, logger, test_env, epoch, step, ar, test, start_time, epoch_fps):
        ...
        if self.env_name.startswith('carla'):
