...
class CurlAgent(AGENTBase):
    ...
    def __init__(self, aug_func, extr_lr, extr_beta, config, **kwargs):
        ...
        # Setting modules
        self._init_extractor(self.env_name, obs_shape, config['extr_params'])
        ...
        self.aux_task = init_auxiliary_task(config['aux_task'],
                                            ...
                                            self.device)
        self.aux_task_name = config['auxiliary']
    ...
    def _update(self, data, logger, step, save_log):
        ...
        # r = data['rew'].squeeze()
        # r2 = data['rew2'].squeeze()
        # r = r.repeat(self.num_aug)
        # r2 = r2.repeat(self.num_aug)
        # aug_r = r + (r - r2)
