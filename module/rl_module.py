...
class SGMLPActor(nn.Module):
    ...
    def act_with_z(self, state): # deterministic=True, state_dim=50
        h = self.pi_trunk(state)
        mu = self.pi_mean(h)
        mu = torch.tanh(mu)
        return self._output(mu)
...
class EnsembleCritic(nn.Module):
    ...
    def __init__(self, action_shape, hidden_dim, repr_dim, encoder_feature_dim, l=2,
        ...
        #self.a_fc = mlp(action_shape[0], 0, action_shape[0], 0, output_mod)
        self.q = mlp(
...
class RRAPredictor(nn.Module):
    def __init__(self, latent_state_dim, hidden_dim, action_dim):
        super(RRAPredictor, self).__init__()

        # Initialize modules
        self.predict_a = nn.Sequential(
            nn.Linear(latent_state_dim*2, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, action_dim))
        
        self.predict_r = nn.Sequential(
            nn.Linear(latent_state_dim+action_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, s, s2):
        return self.predict_a(torch.cat([s, s2], dim=-1))
    
    def r_predictor(self, s, a):
        return self.predict_r(torch.cat([s, a], dim=-1))
