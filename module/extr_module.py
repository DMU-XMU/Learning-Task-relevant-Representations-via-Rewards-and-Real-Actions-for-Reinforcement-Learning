...
class PixelExtractorCarla096(nn.Module):
    """Convolutional encoder of pixels observations."""
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, num_fc=1, *args):
        super().__init__()
        ...
        out_dim = 100 # if defaults change, adjust this as needed

        self.repr_dim = num_filters * out_dim
        if self.is_fc:
            self.fc = nn.Linear(self.repr_dim, self.feature_dim)
            self.ln = nn.LayerNorm(self.feature_dim)
        self.infos = dict()
...
def make_extr(env_name, obs_shape, extr_latent_dim, num_layers, num_filters, num_fc, **args):
    if env_name.startswith('carla'):
        return PixelExtractorCarla096(obs_shape, extr_latent_dim, num_layers, num_filters, num_fc)
