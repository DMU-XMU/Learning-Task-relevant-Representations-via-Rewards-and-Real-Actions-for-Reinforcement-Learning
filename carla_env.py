...
class CarlaEnv(object):
    ...
    def reset_other_vehicles(self):
        ...
        spawn_points = self.map.get_spawn_points()  #每一个地图都会提供一组可用坐标点
        ...
        for _ in range(num_vehicles):
            ...
            choice_point = random.choice(spawn_points)
            spawn_points.remove(choice_point)
            location = choice_point.location
            transform = carla.Transform(location, carla.Rotation(yaw=-90))
